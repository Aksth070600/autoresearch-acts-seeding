...
def flatten_summary(summary: dict[str, Any], path: Path, records_root: Path) -> dict[str, Any] | None:
    ...
    if (
        ...
        and timed_comparison.get("events") == PROTOCOL_METADATA["timing_events"]
    ...
    if not set(PRIMARY_REPORT_METRICS).issubset(metrics):
        return None
...
def active_summary_paths(
    ...
    for path in sorted(records_root.glob("**/summary.json")):
        ...
        if category == dataset and is_compatible_summary(summary):
            paths.append(path)
    return paths


def load_records(records_root: Path, dataset: str) -> list[dict[str, Any]]:
    """Load exact, complete active v3 summaries."""
...
def metric_label(key: str) -> str:
    labels = {
        ...
        RSS_DISPLAY_METRIC: "Raw seeding-only Peak RSS (KiB)",
...
def aggregate_genesis(
    ...
    active v3 summaries by the time they reach this function.
    """

    scoped_rows = [
        row
        for row in rows
        if str(row["category"]).lower() == dataset
...
def build_report(
    ...
    return {
        ...
        "protocol": dict(PROTOCOL_METADATA),
...
def main() -> int:
    ...
    summary_paths = active_summary_paths(records_root, args.dataset)
    if rows or summary_paths:
        ...
        for path in summary_paths:
            ...
            if (
                ...
                and is_compatible_summary(summary)
