...
ProcessCode GridTripletSeedingAlgorithm::execute(
  ...
  const auto gridSpacePointRange = [&](std::size_t bin) {
    return Acts::SpacePointIndexRange2{gridSpacePointOffsets.at(bin),
                                      gridSpacePointOffsets.at(bin + 1)};
  };
  ...
  const Acts::Range1D<float> rRange = [&]() -> Acts::Range1D<float> {
    ...
    for (std::size_t i = 0; i < grid.numberOfBins(); ++i) {
      const Acts::SpacePointIndexRange2 range = gridSpacePointRange(i);
  ...
  for (const auto [bottom, middle, top] : grid.binnedGroup()) {
    ...
    for (const auto b : bottom) {
      bottomSpRanges.push_back(
          coreSpacePoints.range(gridSpacePointRange(b)).asConst());
    }
    middleSpRange =
        coreSpacePoints.range(gridSpacePointRange(middle)).asConst();
    topSpRanges.clear();
    for (const auto t : top) {
      topSpRanges.push_back(
          coreSpacePoints.range(gridSpacePointRange(t)).asConst());
    }
