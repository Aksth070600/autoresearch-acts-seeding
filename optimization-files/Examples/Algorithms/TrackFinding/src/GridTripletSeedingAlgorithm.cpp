  ...
  for (const auto [bottom, middle, top] : grid.binnedGroup()) {
    ...
    for (const auto b : bottom) {
      bottomSpRanges.push_back(
          coreSpacePoints.range(gridSpacePointRanges[b]).asConst());
    }
    middleSpRange =
        coreSpacePoints.range(gridSpacePointRanges[middle]).asConst();
    topSpRanges.clear();
    for (const auto t : top) {
      topSpRanges.push_back(
          coreSpacePoints.range(gridSpacePointRanges[t]).asConst());
    }
