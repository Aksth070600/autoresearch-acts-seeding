...
class Impl final : public TripletSeedFinder {
 public:
  ...
  void createPixelTripletTopCandidates(
    ...
    using StagedTopDoublet =
        std::remove_cvref_t<decltype(*topDoublets.begin())>;
    struct SlopeCompatibleTop {
      StagedTopDoublet topDoublet;
      std::size_t index;
      float error2;
    ...
    for (auto [topDoublet, topDoubletIndex] :
      ...
      slopeCompatible.push_back(
          {topDoublet, topDoubletIndex, error2, deltaCotTheta2});
    }

    for (const SlopeCompatibleTop& candidate : slopeCompatible) {
      const std::size_t topDoubletIndex = candidate.index;
      const auto topDoublet = candidate.topDoublet;
