...
void BroadTripletSeedFilter::filterTripletTopCandidates(
  ...
  for (std::uint32_t i = 0; i < tripletTopCandidates.size(); ++i) {
    cache().topSpCurvatureVec[i] = {tripletTopCandidates.curvatures()[i], i};
  }
  ...
  for (const Cache::CurvatureIndex& top : cache().topSpCurvatureVec) {
    ...
    for (std::size_t variableCompTopIndex = beginCompTopIndex;
         variableCompTopIndex < cache().topSpCurvatureVec.size();
         variableCompTopIndex++) {
      const Cache::CurvatureIndex& compatible =
          cache().topSpCurvatureVec[variableCompTopIndex];
      std::size_t compatibleTopSpIndex = compatible.index;
