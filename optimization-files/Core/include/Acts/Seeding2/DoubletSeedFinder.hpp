...
class DoubletsForMiddleSp {
 public:
  ...
  const boost::container::small_vector<SpacePointIndex2, 64>& spacePoints()
      const {
    return m_spacePoints;
  }
  /// Get reference to cotTheta values container
  /// @return Const reference to cotTheta values vector
  const boost::container::small_vector<float, 64>& cotTheta() const {
    return m_cotTheta;
  }
 ...
 private:
  boost::container::small_vector<SpacePointIndex2, 64> m_spacePoints;

  // parameters required to calculate a circle with linear equation
  boost::container::small_vector<float, 64> m_cotTheta;
