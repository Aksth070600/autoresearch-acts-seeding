...
class BroadTripletSeedFilter final : public ITripletSeedFilter {
 public:
  ...
  struct Config {
    ...
    float absDeltaEtaWeightFactor = 0;
    /// Minimum impact parameter to apply the abs(delta-eta) weight
    /// only want to apply to displaced seeds
    float absDeltaEtaMinImpact = 2 * UnitConstants::mm;
  ...
  struct Cache {
    ...
    /// Curvature-compatible radii awaiting deterministic radial selection
    std::vector<float> compatibleCandidateR;
    /// Cache for selected compatible seed radii during score calculation
