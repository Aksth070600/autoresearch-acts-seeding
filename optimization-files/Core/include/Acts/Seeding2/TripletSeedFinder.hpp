...
class TripletTopCandidates {
 public:
  ...
  /// Fields repeatedly consumed together by curvature sorting and compatible
  /// top-space-point scans.
  struct TopAndCurvature {
  ...
  Index size() const { return static_cast<Index>(m_topAndCurvatures.size()); }
  ...
  void reserve(Index size) {
    m_topAndCurvatures.reserve(size);
  ...
  void emplace_back(SpacePointIndex2 spT, float curvature,
    ...
    m_topAndCurvatures.push_back({spT, curvature});
    m_impactParameters.emplace_back(impactParameter);
  }

  /// Return the hot search fields for one candidate.
  const TopAndCurvature& topAndCurvature(Index index) const {
    return m_topAndCurvatures[index];
  }
  ...
  class Proxy {
   public:
    ...
    SpacePointIndex2 spacePoint() const {
      return m_container->m_topAndCurvatures[m_index].topSpacePoint;
    }

    /// Get the curvature estimation
    /// @return The curvature value
    float curvature() const {
      return m_container->m_topAndCurvatures[m_index].curvature;
    }
 ...
 private:
  std::vector<TopAndCurvature> m_topAndCurvatures;
